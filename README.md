...
bid_floor.py            清算成本分解:闪贷费/gas/竞价/利润 按机会规模分档,量竞价比例
liquidator_scan.py      横扫全市场清算人:多协议清算事件 → 规模 × 给构建者付款
                        ⚠️ 必须先量规模 —— 80% 的清算是 $10 以下的尘埃单
...
docs/week_2/
  ...
  ├── 清算生意原理教学.md       ★ 按逻辑重排:机制 / 经济学 / 五个测量陷阱 / 错误模式
  ├── 论文与实测对照.md         4 篇论文 vs 实测:三条结论被改写、一条被独立印证
  ├── 研究人教学.md             同一课题的纠错日志(按时间),留证据轨迹用
