...
一次报价只是**一个采样点**,而且可能严重偏离常态。
实测:ARB.USDC⇄BAS.WETH 往返,早期 2–3 次采样报 22–43 bps,
98 次观测后中位是 **60.45 bps** —— 少量采样把门槛低估了一半。
跨资产路径的门槛在 [51.90, 73.31] 之间跳动,
...
跨资产路径可以密一些(它是活的),但注意 **`/quote` 未认证限速是 75 次 / 2 小时**
(不是 100 次/60 秒 —— 那是 `/chains` `/token` 这类便宜端点的额度)。

按现在的配置算:稳定币路径每轮 2 次 quote、跨资产往返每轮 4 次,
30 分钟一轮 → 两小时用掉约 24 次,占额度三分之一。**剩下的要留给临时查询。**
想跑更密就要申请 API Key。
